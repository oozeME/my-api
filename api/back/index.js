var query =require("../../config/dbc_connect")
...
router.post("/addlist", function(req, res, next) {
    var oneid = generateUUID()
        //    一级分类英文标识sql查询
    var sqltest1 = `select id from one_class where enname='${req.body.params.enameF}'`
        // 二级分类英文标识sql查询
    var sqltest2 = `select id,article_num from two_class where enname='${req.body.params.enameC}'`
        // 一级分类插入数据sql
    var sqlinsert1 = `insert into one_class(id,enname,cnname,time) values('${oneid}','${req.body.params.enameF}','${req.body.params.cnameF}','${creatTime()}')`
        // 二级分类插入数据sql
    var sqlinsert2 = `insert into two_class(id,parent_id,enname,cnname,article_num,time) values('${generateUUID()}','${oneid}','${req.body.params.enameC}','${req.body.params.cnameC}',0,'${creatTime()}')`
        // 创建文章表
...
//  return new Promise(function(resolve, reject) {
//      query(sql.length, function(err, rows, fields) {
//          if(rows> 0){
//          	resolve("ok")
//          }else {
//              reject("err")
...
//}
//router.post("/addlist", function(req, res, next) {
//  var sqltest1 = `select ename from firstlist where ename='${req.body.params.enameF}'`;
//  var sqltest2 = `select ename from secondlist where ename='${req.body.params.enameC}'`;
//  var sqlinsert1 = `insert into firstlist(id,cname,ename) values('${req.body.params.ids}','${req.body.params.cnameF}','${req.body.params.enameF}')`;
//  var sqlinsert2 = `insert into secondlist(id,parentId,cname,ename) values('${req.body.params.fn}','${req.body.params.ids}','${req.body.params.cnameC}','${req.body.params.enameC}')`;
